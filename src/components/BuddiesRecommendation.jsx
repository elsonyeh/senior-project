...
import buddiesEventService from "../services/buddiesEventService";
...
export default function BuddiesRecommendation({
  ...
  const handleFinishSwiping = useCallback(async () => {
    ...
    // 記錄用戶完成滑動事件
    await buddiesEventService.logSwipingCompleted(roomId, userId, limitedRestaurants.length);
    ...
    if (completedResult.success) {
      ...
      logger.debug("📊 滑動完成度檢查:", {
        ...
        completedUsers: completedResult.userIds,
        allMembersFinished: actualCompletedCount >= totalMembers && totalMembers > 0
      });
      ...
      if (actualCompletedCount < totalMembers) {
        logger.debug("⏳ 還有成員未完成滑動，進入等待狀態", {
          remaining: totalMembers - actualCompletedCount,
          waitingFor: totalMembers - actualCompletedCount
        });
        setWaiting(true); // 顯示等待畫面
        return; // 提前返回，不執行後續的結果計算
      }

      logger.debug("✅ 所有成員都已完成滑動，開始計算最終結果");
    }
  ...
  // 使用 useMemo 計算最終顯示的餐廳列表，避免每次渲染都重新計算
  const finalDisplayData = React.useMemo(() => {
    ...
    return { finalSaved };
  }, [phase, finalResult, votes, saved, limitedRestaurants]);

  // 如果處於結果階段
  if (finalDisplayData) {
    const { finalSaved } = finalDisplayData;
