...
function recommendForGroup(allUserAnswers, restaurants, options = {}) {
  ...
  // 如果沒有房主ID，記錄警告
  if (!hostId) {
    console.warn('沒有提供房主ID，可能影響投票結果處理');
  }
  ...
  // 處理所有用戶的答案
  Object.entries(allUserAnswers).forEach(([userId, userAnswers]) => {
    // 確保答案是陣列
    ...
    answers.forEach((answer, index) => {
      ...
      // 初始化這個問題的投票統計
      if (!votes.has(index)) {
        votes.set(index, new Map());
      }

      // 更新投票計數
      const questionVotes = votes.get(index);
      questionVotes.set(answer, (questionVotes.get(answer) || 0) + 1);

      // 如果是房主的投票，特別記錄
      if (userId === hostId) {
        hostVotes.set(index, answer);
        console.log(`記錄房主選擇 - 題目 ${index}: ${answer}`);
      }

      // 記錄問題的所有答案
      if (!questionAnswers.has(index)) {
        questionAnswers.set(index, new Set());
      }
      questionAnswers.get(index).add(answer);
    });
  });

  // 處理每個問題的投票結果
  const finalAnswers = [];
  let eatOrDrinkChoice = null;

  // 首先處理吃喝問題
  const eatDrinkIndex = Array.from(questionAnswers.keys()).find(index => {
    const questionText = options.questionTexts?.[index] || '';
    return questionText.includes('想吃正餐還是想喝飲料');
  });

  if (eatDrinkIndex !== undefined) {
    const eatDrinkVotes = votes.get(eatDrinkIndex);
    const hostChoice = hostVotes.get(eatDrinkIndex);

    // 找出最高票數的選項
    let maxVotes = 0;
    let winners = [];
    eatDrinkVotes.forEach((count, choice) => {
      if (count > maxVotes) {
        maxVotes = count;
        winners = [choice];
    ...
    // 如果有平票且房主有投票，直接使用房主的選擇
    if (winners.length > 1 && hostChoice) {
      eatOrDrinkChoice = hostChoice;
      console.log('平票情況，使用房主選擇:', hostChoice);
    } else {
      eatOrDrinkChoice = winners[0];
    }

    finalAnswers[eatDrinkIndex] = eatOrDrinkChoice;
    console.log(`吃喝選擇確定: ${eatOrDrinkChoice} (房主選擇: ${hostChoice || '無'})`);
  }

  // 處理其他問題
  votes.forEach((questionVotes, index) => {
    // 如果已經處理過這個問題，跳過
    if (finalAnswers[index] !== undefined) return;

    // 如果選擇了喝，跳過吃相關的問題
    const questionText = options.questionTexts?.[index] || '';
    if (eatOrDrinkChoice === '喝' &&
      (questionText.includes('吃一點還是吃飽') ||
        questionText.includes('想吃辣的還是不辣'))) {
      console.log(`跳過吃相關問題 ${index}: ${questionText}`);
      return;
    }

    let maxVotes = 0;
    let winners = [];
    const hostChoice = hostVotes.get(index);

    // 統計票數
    questionVotes.forEach((count, choice) => {
      if (count > maxVotes) {
        maxVotes = count;
        winners = [choice];
      } else if (count === maxVotes) {
    ...
    console.log(`題目 ${index} 投票結果:`, {
      winners,
      hostChoice,
      voteCounts: Array.from(questionVotes.entries())
    });

    // 如果有平票且房主有選擇，直接使用房主的選擇
    if (winners.length > 1 && hostChoice) {
      finalAnswers[index] = hostChoice;
      console.log(`題目 ${index} 平票情況，使用房主選擇:`, hostChoice);
    } else {
      finalAnswers[index] = winners[0];
      console.log(`題目 ${index} 選擇確定:`, winners[0]);
    }
  });

  // 移除空值並壓縮數組
  const compactAnswers = finalAnswers.filter(answer => answer !== undefined);
  console.log('最終答案:', compactAnswers);

  // 使用修改後的答案進行推薦
  return recommendRestaurants(compactAnswers, restaurants, {
    ...options,
    seed,
    hostId,
