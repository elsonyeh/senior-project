...
**地圖位置持久化** ⭐ NEW (v2.1.0)
- **智能定位策略**：
  - 首次訪問：自動請求用戶位置（高精確度模式）
  - 再次訪問：從 localStorage 恢復上次瀏覽位置
  - 頁面導航/重整：保持原位不重新定位
- **localStorage 存儲**：
  ```javascript
  {
    ...
    lng: 121.5654,       // 地圖中心經度
    zoom: 16,            // 縮放級別
    timestamp: 1698912000000  // 保存時間
  }
  ```
- **防抖優化**：
  - MapView 使用 `idle` 事件自然防抖
  - MapPage 使用 500ms 防抖避免頻繁寫入
- **錯誤處理**：
  - Projection 可用性檢查（避免 undefined 錯誤）
  - 降級策略：projection 不可用時使用直接居中

**智能定位系統** ⭐ NEW (v2.1.0)
- **定位精確度階層**：
  - 優秀：< 15m（1-2 秒完成）
  - 良好：15-50m（收集 4 個樣本）
  - 可接受：50-100m（收集 8 個樣本）
  - 降級：> 100m（10 秒超時後使用最佳樣本）
- **多樣本平滑算法**：
  ```javascript
  // 加權平均，精確度越高權重越大
  weight = 1 / accuracy
  smoothedPosition = Σ(position × weight) / Σ(weight)
  ```
- **超時降級機制**：
  - 10 秒強制超時保護
  - 使用平方倒數降低差樣本影響
